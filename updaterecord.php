...
if(isset($_REQUEST["pno"]) && $_REQUEST["pno"]!=""){
	$patient=$_REQUEST["pno"];
	if(searchRecord("patientrecord","pno",$patient)==0){
		header("location: update.php?mess=Invalid Patient No");
	}
}else{
	...
	if(isset($_REQUEST["update"])){
		$pno=$_REQUEST["pno"];
		$sql="UPDATE patientrecord SET name='$_REQUEST[pname]', tel='$_REQUEST[tel]', address='$_REQUEST[add]', ptype='$_REQUEST[faculty]', lnp='$_REQUEST[lmp]', sp='$_REQUEST[dept]', dob='$_REQUEST[dob]' WHERE pno='$pno'";
		if(result($sql)){
			$mess="Patient Record Successfully Updated [<a href=\"schedule.php?pno=$pno\" target=\"_blank\">Print Patient Schedule</a>]";
		}
	}
?>
<!DOCTYPE HTML>
...
<h1 style="margin: 5px 0px 0px 5px; font-size: 30px; letter-spacing: 3px;"><img src="images/logo.png" /><br> SPECIALIST HOSPITAL & FERTILITY CENTER</h1>
	...
	<?php
		if(isset($patient)){
			$rec=getRecs("patientrecord","pno",$patient);
		}	
	?>
	<h2>Patient Schedule Generation</h2>
			...
			<div class="gaps">
				<p>PATIENT NUMBER</p>
					<input type="text" name="pno" <?php echo 'value="'.$rec[1].'"'; ?> placeholder="" required=""/>
			</div>
			<div class="gaps">
				<p>PATIENT NAME</p>
					<input type="text" name="pname" <?php echo 'value="'.$rec[2].'"'; ?> placeholder="" required=""/>
			</div>
				<div class="gaps">	
				<p>PHONE NUMBER</p>
					<input type="text" name="tel" <?php echo 'value="'.$rec[3].'"'; ?> placeholder="" required=""/>
				</div>
			<div class="gaps">
				<p>ADDRESS</p>
					<input type="text" name="add" <?php echo 'value="'.$rec[4].'"'; ?> placeholder="" required=""/>
			</div>
			</div>
			<div class="right-agileinfo same">
			<div class="gaps">
				<p>PROTOCOL TYPE</p>	
					<select class="form-control" name="faculty" id="faculty-list" class="demoInputBox form-control" onChange="getDept(this.value);">
						<option></option>
						<option <?php if($rec[5]=="LONG"){ echo " selected "; } ?> >LONG</option>
						<option <?php if($rec[5]=="SHORT"){ echo " selected "; } ?> >SHORT</option>
						<option <?php if($rec[5]=="RECIEPIENT(NON MENSTRATING)"){ echo " selected "; } ?> >RECIEPIENT(NON MENSTRATING)</option>
						<option <?php if($rec[5]=="RECIEPIENT(MENSTRATING)"){ echo " selected "; } ?> >RECIEPIENT(MENSTRATING)</option>
					</select>
			</div>
			<div class="gaps">
				<p>STARTING POINT</p>	
